...
import { FileText, Loader2, Printer, Share2, X } from "lucide-react";

import { ServiceOrder, serviceOrderApi } from "@/lib/service-order";
...
export default function ServiceOrderInvoicePage() {
  const router = useRouter();
  const searchParams = useSearchParams();

  const orderId = searchParams.get("id") || "";
  const [orderData, setOrderData] = useState<ServiceOrder | null>(null);
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      ...
      try {
        ...
        console.error("Gagal memuat invoice service order:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleWhatsApp = () => {
    ...
    const cleanPhone = normalizePhoneToWhatsApp(orderData.customer.phone);
    if (!cleanPhone) {
    ...
    const detailLines =
      orderData.details?.map((detail) => {
        const name = detail.service?.name || `Layanan #${detail.service_id}`;
        const quantity = Number(detail.quantity ?? detail.qty) || 0;
        const subtotal = Number(detail.subtotal) || Number(detail.price) * quantity;
        return `- ${name} x${quantity} (${formatRupiah(subtotal)})`;
      }) || [];

    const subtotal = (orderData.details || []).reduce((sum, detail) => {
      const quantity = Number(detail.quantity ?? detail.qty) || 0;
      return sum + (Number(detail.subtotal) || Number(detail.price) * quantity);
    }, 0);
    const totalDiscount = (subtotal * (Number(orderData.discount) || 0)) / 100;
    const total = subtotal - totalDiscount;

    const message = [
      `Halo ${orderData.customer?.name || "Customer"},`,
      "",
      "Terima kasih sudah menggunakan layanan kami. Berikut invoice service order Anda:",
      "",
      "*INVOICE SERVICE ORDER*",
      `No. Order: *${orderData.order_number || orderId}*`,
      `Status: *${formatStatusText(orderData.status || "")}*`,
      "",
      "*Detail Layanan*",
      ...detailLines,
      "",
      ...
      "",
      "Jika ada pertanyaan terkait order ini, silakan balas pesan ini.",
      "",
      "Terima kasih.",
    ].join("\n");
  ...
  const items =
    orderData.details?.map((detail) => {
      ...
      return {
        name: detail.service?.name || `Layanan #${detail.service_id}`,
        qty: quantity,
        price,
        subtotal: Number(detail.subtotal) || price * quantity,
      };
    }) || [];
        ...
        >
          <div className="p-8 border-b border-gray-50">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h1 className="text-xl font-bold text-gray-900">
                  Invoice Service #{orderData.order_number || orderId}
                </h1>
            ...
            <div className="space-y-1">
              ...
              <p className="pt-2 text-xs font-semibold text-blue-600">
                Status: {formatStatusText(orderData.status || "")}
              </p>
              {orderData.notes && (
                <p className="text-xs text-gray-500">Catatan: {orderData.notes}</p>
              )}
            </div>
          </div>
